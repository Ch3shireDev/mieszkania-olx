import sys
import os
import shutil
from datetime import datetime
import pysftp
import paramiko
import glob
from config import server, username, password


try:
    if os.path.exists('deploy'):
        shutil.rmtree('deploy')
    os.mkdir('deploy')
except:
    print("Error removing ./deploy directory. Aborting...")
    exit()

try:
    os.system('ng build --prod')
    shutil.move('./dist', 'deploy')
except:
    print('Error compiling Angular project. Aborting...')
    exit()


try:
    shutil.copy('./server.js', 'deploy')
except Exception as e:
    print(f'Error compiling Express.js project: {e}. Aborting...')
    exit()

print("Creating archive directory.")

try:
    now = datetime.now()
    dt = now.strftime("%Y%m%dT%H%M")
    fname = f'deploy-{dt}'
    shutil.make_archive(fname, 'zip', 'deploy')
    fname += '.zip'
    shutil.rmtree('deploy')
except Exception as e:
    print(f'Error during archving build directory: {e}. Aborting...')
    exit()

try:
    print("Connecting to ftp.")
    srv = pysftp.Connection(host="zadania.org.pl", username=username,
                            password=password)
    print("Sending file to ftp.")
    srv.put(fname)
    srv.close()
except Exception as e:
    print(f"Error connecting to SSH: {e}")

client = paramiko.SSHClient()
client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
client.connect(server, username=username, password=password)
client.exec_command('python3 update-olx.py')

for file in glob.glob('deploy-*.zip'):
    os.remove(file)
